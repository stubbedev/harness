...
The snapshot reads the entire checkout, including ignored dependency/build
folders. Large checkouts can therefore take significant time and disk space.
Sensitive ignored files also appear in the child snapshot: use private storage.
Nested repositories, submodules, special files, unstable snapshots, non-Git
folders, and repositories without a first commit fail rather than risk an
incomplete overlay. No fallback to a shared checkout occurs.
...
A changed worktree is retained. Results report its absolute path, branch, and a
binary-capable `changes-*.patch` artifact. That patch compares the child's final
