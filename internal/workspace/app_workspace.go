...
// LSPFileDiagnostics folds every running server's diagnostics into per-file
// severity counts. Servers publish whole files at a time, so a file missing
// from the map genuinely has no diagnostics.
