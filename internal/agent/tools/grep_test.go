...
func TestGrepWithIgnoreFiles(t *testing.T) {
	...
	harnessignoreContent := "node_modules/\n"
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, ".harnessignore"), []byte(harnessignoreContent), 0o644))
	...
	for name, fn := range map[string]func(pattern, path, include string) ([]grepMatch, error){
		...
		t.Run(name, func(t *testing.T) {
			...
			require.False(t, foundFiles["lib.js"], "Should not find lib.js (ignored by .harnessignore)")
...
func TestSearchImplementations(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, ".harnessignore"), []byte("file5.txt\n"), 0o644))
