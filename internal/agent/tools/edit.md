...
Read the affected text with `view` or an LSP source snippet before editing. Changed file contents are refused even if the timestamp is unchanged. A refusal returns bounded current evidence for an explicit retry; it never applies the stale edit. Partial edits do not certify unseen parts of the file.
