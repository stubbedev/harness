...
// one. When local, `harness run --channels server:webhook` fails with
...
func TestChannelsFlagAvailableOnRunCmd(t *testing.T) {
	...
	require.NotNil(t, flag, "the --channels flag must be available on `harness run` (register it as a persistent flag on rootCmd)")
...
func TestChannelsFlagAvailableOnRootCmd(t *testing.T) {
	...
	require.NotNil(t, flag, "the --channels flag must be available on `harness` (rootCmd)")
}
