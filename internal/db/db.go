...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.getMemoryByTitleStmt, err = db.PrepareContext(ctx, getMemoryByTitle); err != nil {
		return nil, fmt.Errorf("error preparing query GetMemoryByTitle: %w", err)
	}
	...
	if q.reapMemoriesStmt, err = db.PrepareContext(ctx, reapMemories); err != nil {
		return nil, fmt.Errorf("error preparing query ReapMemories: %w", err)
	}
...
func (q *Queries) Close() error {
	...
	if q.getMemoryByTitleStmt != nil {
		if cerr := q.getMemoryByTitleStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing getMemoryByTitleStmt: %w", cerr)
		}
	}
	...
	if q.reapMemoriesStmt != nil {
		if cerr := q.reapMemoriesStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing reapMemoriesStmt: %w", cerr)
		}
	}
