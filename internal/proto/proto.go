...
// UpdateAvailable is published when a newer Harness release is detected
...
// completion contract (notably `harness run` in client/server mode)
...
// `harness run`, which may attach to a busy session whose currently
