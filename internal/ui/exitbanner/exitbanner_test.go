...
func TestRender(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			banner: config.ExitBannerCompact,
			sess:   testSession(),
			want:   []string{"Session", "Fix the flaky test", "Continue", "harness -s "},
			// No logo, and no padding around the two lines.
			notWant: []string{"Thanks for using Harness!"},
		},
		{
			name:   "default with a session prints the logo and the resume lines",
			banner: config.ExitBannerDefault,
			sess:   testSession(),
			want:   []string{"Thanks for using Harness!", "Session", "Fix the flaky test", "Continue"},
		},
		{
			name:   "default without a session still prints the logo",
			banner: config.ExitBannerDefault,
			want:   []string{"Thanks for using Harness!"},
		...
		},
		{
			...
			sess:   testSession(),
			want:   []string{"Thanks for using Harness!", "Continue"},
		},
		{
