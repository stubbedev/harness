...
import (
	...
	"github.com/stubbedev/harness/internal/ui/diffview"
)
...
const (
	...
	// it a plain no-break space without the text-presentation variation
	// selector upstream appends: several terminals render the bare U+FE0E
	// as a visible glyph instead of composing it into the blank, which
	// litters every inline code span with tofu.
...
type Styles struct {
	...
	// Harness's background. Defining them here keeps output readable and
	...
	Header struct {
		...
		LogoGradCanvas    lipgloss.Style // Canvas for the compact "HARNESS" gradient
		LogoGradFromColor color.Color    // "HARNESS" wordmark gradient start
		LogoGradToColor   color.Color    // "HARNESS" wordmark gradient end
	...
	Tool struct {
		...
		// Tool name styles, colored by call status. Ordinary states
		// (running, done) share the understated grey so tool calls recede
		// behind the chat messages; only failure states keep a color.
		NamePending   lipgloss.Style // Running or awaiting permission (grey)
		NameNormal    lipgloss.Style // Done/success, top-level tool name (grey)
		NameNested    lipgloss.Style // Done/success, nested child tool name (grey)
