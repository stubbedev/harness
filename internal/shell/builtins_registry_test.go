...
func TestRegisterBuiltin_DispatchedThroughRun(t *testing.T) {
	RegisterBuiltin("harnesstest-echo", func(ctx context.Context, args []string, stdin io.Reader, stdout, stderr io.Writer) error {
		io.WriteString(stdout, "builtin:"+args[1])
		return nil
	})
	t.Cleanup(func() { delete(builtins, "harnesstest-echo") })

	var stdout bytes.Buffer
	err := Run(t.Context(), RunOptions{
		Command: "harnesstest-echo hello",
		Cwd:     t.TempDir(),
		Stdout:  &stdout,
	})
	if err != nil {
...
func TestRegisterBuiltin_OverridesPATHBinary(t *testing.T) {
	dir := t.TempDir()
	// Put a fake "harnesstest-shadowed" binary on PATH that would print
	// "from-path" if it ran.
	binDir := filepath.Join(dir, "bin")
	if err := os.MkdirAll(binDir, 0o755); err != nil {
		t.Fatal(err)
	}
	script := filepath.Join(binDir, "harnesstest-shadowed")
	if err := os.WriteFile(script, []byte("#!/bin/sh\necho from-path\n"), 0o755); err != nil {
		t.Fatal(err)
	}
	t.Setenv("PATH", binDir+string(os.PathListSeparator)+os.Getenv("PATH"))

	RegisterBuiltin("harnesstest-shadowed", func(ctx context.Context, args []string, stdin io.Reader, stdout, stderr io.Writer) error {
		io.WriteString(stdout, "from-builtin")
		return nil
	})
	t.Cleanup(func() { delete(builtins, "harnesstest-shadowed") })

	var stdout bytes.Buffer
	err := Run(t.Context(), RunOptions{
		Command: "harnesstest-shadowed",
		Cwd:     dir,
		Env:     os.Environ(),
		Stdout:  &stdout,
	})
