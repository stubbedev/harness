...
// ptyColorEnvVars force color output for captured runs: programs that
// respect FORCE_COLOR or CLICOLOR_FORCE emit ANSI color sequences even
// without a real PTY. The plain [Run] path does not set them, since
// there ANSI codes would be noise.
...
// Harness handler stack. Shared by the stateless [Run] entrypoint and the
...
// standard Harness middleware chain (builtins, script dispatch, block list)
...
// when sourcing framework files can send SIGINT/SIGTERM to Harness's process
...
// regardless of the caller's environment because Harness shells are never
...
//  1. builtins first (so Harness's in-process jq wins over any PATH binary);
...
// builtinHandler returns middleware that dispatches recognized Harness
// builtins to their in-process Go implementations. Every builtin lives in
// the builtins map.
