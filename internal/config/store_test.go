...
func TestConfigStore_ConfigPath_GlobalAlwaysWorks(t *testing.T) {
	...
	store := &ConfigStore{
		globalDataPath: "/some/global/harness.yaml",
	}

	path, err := store.configPath(ScopeGlobal)
	require.NoError(t, err)
	require.Equal(t, "/some/global/harness.yaml", path)
}
...
func TestConfigStore_ConfigPath_WorkspaceReturnsPath(t *testing.T) {
	...
	store := &ConfigStore{
		workspacePath: "/some/workspace/.harness/harness.yaml",
	}

	path, err := store.configPath(ScopeWorkspace)
	require.NoError(t, err)
	require.Equal(t, "/some/workspace/.harness/harness.yaml", path)
}
...
func TestConfigStore_SetConfigField_GlobalScopeAlwaysWorks(t *testing.T) {
	...
	require.NoError(t, err)
	require.Contains(t, string(raw), "foo: bar", "the file on disk is YAML")
...
func TestReloadFromDisk_UsesNewConfigValues(t *testing.T) {
	...
	t.Setenv("HARNESS_GLOBAL_CONFIG", dir)
	t.Setenv("HARNESS_GLOBAL_DATA", dir)
	resetProviderState()
...
func TestConfigStore_SetConfigFields_concurrentInProcess(t *testing.T) {
	...
	data, err := readConfigJSON(configPath)
	require.NoError(t, err)
