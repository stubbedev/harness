...
// writeTokenToDisk persists token as the fixture provider credential in the
// config file at path, mimicking what another harness instance would leave
...
// newRefreshTestStore builds a ConfigStore whose fixture provider holds an
...
// which lets a single test process faithfully simulate two harness instances:
...
func newRefreshTestStore(t *testing.T, configPath string, exchange func(ctx context.Context, providerID, refreshToken string) (*oauth.Token, error)) *ConfigStore {
	...
	// Keep provider reloads offline: these tests exercise token
	// rotation, not catalog fetching.
	...
	providers.Set("rotator", ProviderConfig{
		ID:         "rotator",
		Name:       "Hyper",
...
func TestRefreshOAuthToken_InProcessSingleFlight(t *testing.T) {
	...
	for range goroutines {
		wg.Go(func() {
			<-start
			errs <- store.RefreshOAuthToken(context.Background(), ScopeGlobal, "rotator")
		})
	}
	close(start)
	wg.Wait()
	...
	pc, ok := store.config.Providers.Get("rotator")
	require.True(t, ok)
...
func TestRefreshOAuthToken_CrossProcessAdopt(t *testing.T) {
	...
	for _, s := range []*ConfigStore{a, b} {
		wg.Go(func() {
			<-start
			errs <- s.RefreshOAuthToken(context.Background(), ScopeGlobal, "rotator")
		})
	}
	close(start)
	wg.Wait()
	...
	for name, s := range map[string]*ConfigStore{"a": a, "b": b} {
		pc, ok := s.config.Providers.Get("rotator")
...
func TestRefreshOAuthToken_StalePeerBorrowsRotatedRefreshToken(t *testing.T) {
	...
	require.NoError(t, store.RefreshOAuthToken(context.Background(), ScopeGlobal, "rotator"))
