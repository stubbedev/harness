...
func TestConfig_configureProvidersBedrockWithCredentials(t *testing.T) {
	...
	bedrockProvider, ok := cfg.Providers.Get(string(catalog.InferenceProviderBedrock))
...
func TestConfig_configureProvidersVertexAIWithCredentials(t *testing.T) {
	...
	vertexProvider, ok := cfg.Providers.Get(string(catalog.InferenceProviderVertexAI))
...
func TestConfig_setupAgentsWithNoDisabledTools(t *testing.T) {
	...
	require.True(t, ok)
	assert.Equal(t, []string{"batch", "shell", "edit", "lsp_symbols", "lsp_definition", "lsp_call_hierarchy", "web_search", "send_message", "view", "write"}, taskAgent.AllowedTools)
}
...
func TestConfig_setupAgentsWithDisabledTools(t *testing.T) {
	...
	assert.Equal(t, []string{"agent", "wait", "batch", "shell", "harness_info", "harness_logs", "job_output", "job_kill", "lsp_diagnostics", "lsp_references", "lsp_restart", "lsp_symbols", "lsp_definition", "lsp_call_hierarchy", "lsp_rename", "lsp_replace_symbol", "research", "memory", "question", "web_search", "send_message", "todos", "view", "write", "list_mcp_resources", "read_mcp_resource"}, coderAgent.AllowedTools)

	taskAgent, ok := cfg.Agents[AgentTask]
	require.True(t, ok)
	// edit is in the disabled set, so it drops out of the subagent tool set.
	assert.Equal(t, []string{"batch", "shell", "lsp_symbols", "lsp_definition", "lsp_call_hierarchy", "web_search", "send_message", "view", "write"}, taskAgent.AllowedTools)
}
...
func TestConfig_setupAgentsWithEveryReadOnlyToolDisabled(t *testing.T) {
	...
	coderAgent, ok := cfg.Agents[AgentCoder]
	require.True(t, ok)
	assert.Equal(t, []string{"agent", "wait", "harness_info", "harness_logs", "job_output", "job_kill", "edit", "lsp_diagnostics", "lsp_references", "lsp_restart", "lsp_rename", "lsp_replace_symbol", "fetch", "research", "memory", "question", "todos", "write", "list_mcp_resources", "read_mcp_resource"}, coderAgent.AllowedTools)

	taskAgent, ok := cfg.Agents[AgentTask]
	require.True(t, ok)
	// The write tools survive disabling every read-only tool: the built-in
	// subagent set is read-and-edit, not read-only.
	assert.Equal(t, []string{"edit", "write"}, taskAgent.AllowedTools)
}
...
func TestConfig_configureProvidersEnhancedCredentialValidation(t *testing.T) {
	t.Run("VertexAI provider removed when credentials missing with existing config", func(t *testing.T) {
		...
		cfg := &Config{
			Providers: csync.NewMapFrom(map[string]ProviderConfig{
				string(catalog.InferenceProviderVertexAI): {
					BaseURL: "custom-url",
				},
			}),
		}
		...
		require.Equal(t, cfg.Providers.Len(), 0)
		_, exists := cfg.Providers.Get(string(catalog.InferenceProviderVertexAI))
		require.False(t, exists)
	})
	...
	t.Run("Bedrock provider removed when AWS credentials missing with existing config", func(t *testing.T) {
		...
		cfg := &Config{
			Providers: csync.NewMapFrom(map[string]ProviderConfig{
				string(catalog.InferenceProviderBedrock): {
					BaseURL: "custom-url",
				},
			}),
		}
		...
		require.Equal(t, cfg.Providers.Len(), 0)
		_, exists := cfg.Providers.Get(string(catalog.InferenceProviderBedrock))
		require.False(t, exists)
	})
