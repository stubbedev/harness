...
func TestConfig_setupAgentsWithNoDisabledTools(t *testing.T) {
	...
	require.True(t, ok)
	assert.Equal(t, []string{"batch", "lsp_symbols", "lsp_definition", "lsp_call_hierarchy", "glob", "grep", "ls", "sourcegraph", "send_message", "view"}, taskAgent.AllowedTools)
}
...
func TestConfig_setupAgentsWithDisabledTools(t *testing.T) {
	...
	assert.Equal(t, []string{"agent", "batch", "bash", "harness_info", "harness_logs", "job_output", "job_kill", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "lsp_symbols", "lsp_definition", "lsp_call_hierarchy", "lsp_rename", "lsp_replace_symbol", "fetch", "agentic_fetch", "glob", "ls", "question", "sourcegraph", "send_message", "todos", "view", "write", "list_mcp_resources", "read_mcp_resource"}, coderAgent.AllowedTools)

	taskAgent, ok := cfg.Agents[AgentTask]
	require.True(t, ok)
	assert.Equal(t, []string{"batch", "lsp_symbols", "lsp_definition", "lsp_call_hierarchy", "glob", "ls", "sourcegraph", "send_message", "view"}, taskAgent.AllowedTools)
}
