...
func TestBuildEnv(t *testing.T) {
	...
	require.Equal(t, "ls", envMap["HARNESS_TOOL_INPUT_COMMAND"])
	require.Equal(t, "/tmp/f.txt", envMap["HARNESS_TOOL_INPUT_FILE_PATH"])

	// Shared Harness markers must be present so hook-authored scripts can
	// detect they're running under Harness the same way bash-tool-invoked
	// scripts can.
	require.Equal(t, "1", envMap["HARNESS"])
	require.Equal(t, "harness", envMap["AGENT"])
	require.Equal(t, "harness", envMap["AI_AGENT"])
}
...
func TestParseStdoutClaudeCodeFormat(t *testing.T) {
	...
	t.Run("harness format still works", func(t *testing.T) {
		t.Parallel()
