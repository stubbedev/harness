...
// TestRewindBothWithoutSnapshot pins the degradation of a
// conversation-and-files rewind on a turn with no snapshot: the
// transcript is still rewound, the working tree is left alone, and
// files-only mode keeps its explicit error.
func TestRewindBothWithoutSnapshot(t *testing.T) {
	t.Parallel()
	svc, sess, workDir, _ := newTestService(t)
	...
	// A second turn whose snapshot never landed, the way a failed or
	// unavailable checkpoint leaves it.
	turn2, err := svc.messages.Create(t.Context(), sess.ID, message.CreateMessageParams{
		Role:  message.User,
	...
	require.NoError(t, svc.Rewind(t.Context(), sess.ID, turn2.ID, ModeBoth))
	...
	content, err := os.ReadFile(filepath.Join(workDir, "a.txt"))
	require.NoError(t, err)
	require.Equal(t, "two", string(content), "no snapshot means nothing to restore")

	err = svc.Rewind(t.Context(), sess.ID, turn2.ID, ModeFiles)
	require.Error(t, err)
	require.Contains(t, err.Error(), "no snapshot for this turn")
}
