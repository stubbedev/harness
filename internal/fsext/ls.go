...
// harnessGlobalIgnorePatterns returns patterns from the user's
// ~/.config/harness/ignore file.
var harnessGlobalIgnorePatterns = sync.OnceValue(func() []gitignore.Pattern {
	name := filepath.Join(home.Config(), "harness", "ignore")
	bts, err := os.ReadFile(name)
	if err != nil {
		if !os.IsNotExist(err) {
			slog.Debug("Failed to read harness global ignore file", "path", name, "error", err)
		}
		return nil
	}
...
type directoryLister struct {
	// dirPatterns caches parsed patterns from .gitignore/.harnessignore for each directory.
...
// .gitignore and .harnessignore files. Results are cached.
...
func (dl *directoryLister) getCombinedMatcher(dir string) gitignore.Matcher {
	return dl.combinedMatchers.GetOrSet(dir, func() gitignore.Matcher {
		...
		// Add global ignore patterns (git core.excludesFile + harness global ignore).
		allPatterns = append(allPatterns, gitGlobalIgnorePatterns()...)
		allPatterns = append(allPatterns, harnessGlobalIgnorePatterns()...)
