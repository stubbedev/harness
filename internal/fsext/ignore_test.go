...
func TestHarnessIgnore(t *testing.T) {
	...
	// Create a .harnessignore file that ignores .log files
	require.NoError(t, os.WriteFile(".harnessignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .harnessignore file
	harnessignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".harnessignore"), []byte(harnessignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .harnessignore: %v", err)
	}
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .harnessignore in subdir that ignores normal_nested
	subHarnessignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".harnessignore"), []byte(subHarnessignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .harnessignore: %v", err)
	}
