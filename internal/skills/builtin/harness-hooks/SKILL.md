---
name: harness-hooks
description: Use when the user wants to add, write, debug, or configure a Harness hook — gating or blocking tool calls, approving or rewriting tool input before execution, injecting context into tool results, or troubleshooting hook behavior configured in harness.yaml.
---
...
3. Add the entry under `hooks.PreToolUse` in `harness.yaml` with the right matcher.
...
Harness also accepts Claude Code's `hookSpecificOutput` envelope. One intentional
divergence: Harness treats `updated_input` as shallow-merge, Claude Code replaces.
