...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (harness-config).
	activeSkills := []*Skill{
		{Name: "harness-config", Description: "Harness config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("harness-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via harness://...).
	tracker.MarkLoaded("harness-config")
	require.True(t, tracker.IsLoaded("harness-config"))
