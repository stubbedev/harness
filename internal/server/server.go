...
import (
	...
	_ "github.com/stubbedev/harness/internal/swagger"
...
// socketDir returns the directory used for the Harness Unix socket.
...
// returned by [socketDir] and is named harness-<uid>.sock, falling
// back to harness.sock when the current uid cannot be determined. If
// the composed path would exceed [maxUnixSocketPathLen] bytes (the
// macOS sun_path limit), we fall back to /tmp/harness-<uid>.sock so
// the socket remains bindable.
func DefaultHost() string {
	sock := "harness.sock"
	usr, err := user.Current()
	if err == nil && usr.Uid != "" {
		sock = fmt.Sprintf("harness-%s.sock", usr.Uid)
	}
...
// Server represents a Harness server bound to a specific address.
type Server struct {
...
func (s *Server) installHandler() {
	...
	mux.HandleFunc("GET /v1/workspaces/{id}/sessions/{sid}/checkpoints", c.handleGetWorkspaceSessionCheckpoints)
	...
	mux.HandleFunc("POST /v1/workspaces/{id}/lsps/restart", c.handlePostWorkspaceLSPRestart)
	mux.HandleFunc("POST /v1/workspaces/{id}/lsps/disable", c.handlePostWorkspaceLSPDisable)
	mux.HandleFunc("POST /v1/workspaces/{id}/lsps/enable", c.handlePostWorkspaceLSPEnable)
	...
	mux.HandleFunc("POST /v1/workspaces/{id}/agent/sessions/{sid}/cancel-turn", c.handlePostWorkspaceAgentSessionCancelTurn)
