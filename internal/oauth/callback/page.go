...
func Write(w io.Writer, r Result) error {
	...
	if r.Failed() {
		...
		data.Detail = "Harness was not granted access to"
		if r.Subject == "" {
			data.Detail = "Harness was not granted access."
		}
		// A failed page keeps itself open: the reader needs the reason,
		// and closing the tab out from under them would take it away.
		data.Status = "Close this tab and try again from Harness."
	} else {
		data.Title = "Authorized — Harness"
		data.Kind = "ok"
		data.Heading = "You’re all set"
		data.Detail = "Harness is now connected to"
		if r.Subject == "" {
			data.Detail = "Harness is now connected."
		}
