...
// HARNESS_CALLBACK_PREVIEW to a directory and it writes each page state
// there so the result can be opened in a browser and eyeballed.
func TestWritePreview(t *testing.T) {
	dir := os.Getenv("HARNESS_CALLBACK_PREVIEW")
	if dir == "" {
		t.Skip("set HARNESS_CALLBACK_PREVIEW=<dir> to render preview pages")
	}
